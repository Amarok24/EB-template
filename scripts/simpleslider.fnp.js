/* Simpleslider, https://github.com/ruyadorno/simple-slider */
(function(global, factory) {
  ...
  function defaultEase(time, begin, change, duration) {
    return (time = time / (duration / 2)) < 1 ? change / 2 * time * time * time + begin : change / 2 * ((time -= 2) * time *
      time + 2) + begin;
  }
  ...
  function getSlider(options) {
    ...
    function dispose() {
      ...
      imgs = containerElem = interval = trProp = trTime = delay = startVal = endVal = paused = actualIndex = remainingTime =
        onChange = onChangeEnd = null;
    }
    ...
    function anim(insertElem, insertFrom, insertTo, removeElem, removeFrom, removeTo, transitionDuration, startTime,
      elapsedTime, easeFunc) {
      ...
      requestAnimationFrame(function(time) {
        ...
        anim(insertElem, insertFrom, insertTo, removeElem, removeFrom, removeTo, transitionDuration, startTime, time,
          easeFunc);
      });
    }
